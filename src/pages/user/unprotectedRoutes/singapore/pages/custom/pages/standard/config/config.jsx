// =====================================================
// SINGAPORE - CUSTOM STANDARD
// =====================================================

import singapore from "@/assets/singapore.jpg";

// =====================================================
// PAGE DATA
// =====================================================

export const data = {
  title: "Singapore Custom Standard",

  heroImg: singapore,

  content: [
    "Experience Singapore through a custom journey designed around your interests, filled with modern attractions, delicious food, exciting activities, and unforgettable experiences.",

    "Explore Singapore at your own pace, discover its unique culture, enjoy beautiful landmarks, and create memorable moments with a trip designed for you.",

    "This Standard package offers a comfortable Singapore holiday with a balanced combination of sightseeing, adventure, relaxation, culture, food, and flexibility.",
  ],

  xRange1: ["0%", "-27%"],
  xRange2: ["0%", "-25%"],
  xRange3: ["0%", "-23%"],
  xRange4: ["0%", "-20%"],
  xRangeLast: ["0%", "0%"],

  sliderImg: [
    singapore,
    singapore,
    singapore,
  ],

  price: "55,000",
};

// =====================================================
// ACTIVITIES
// =====================================================

export const activities = [
  {
    day: 0,
    title: "Arrival Day",
    image: singapore,
    color: "DDA15E",
    plans: [
      {
        title: "Rest",
        data: [
          "Relax after your arrival in Singapore",
          "Check in to your hotel and settle down",
        ],
      },
      {
        title: "Activities",
        data: [
          "Explore nearby attractions",
          "Enjoy your first Singaporean meal",
        ],
      },
    ],
  },

  {
    day: 1,
    title: "Singapore City Exploration",
    image: singapore,
    color: "F5CFDE",
    plans: [
      {
        title: "Visit",
        data: [
          "Explore Marina Bay",
          "See the Merlion",
          "Discover Singapore's city centre",
        ],
      },
      {
        title: "Explore",
        data: [
          "Choose attractions based on your interests",
          "Enjoy the city atmosphere",
        ],
      },
    ],
  },

  {
    day: 2,
    title: "Culture and Heritage",
    image: singapore,
    color: "EB7E67",
    plans: [
      {
        title: "Visit",
        data: [
          "Explore Chinatown",
          "Visit Little India",
          "Discover local heritage and architecture",
        ],
      },
      {
        title: "Food",
        data: [
          "Try authentic Singaporean cuisine",
          "Explore local food markets",
        ],
      },
    ],
  },

  {
    day: 3,
    title: "Custom Adventure Day",
    image: singapore,
    color: "FFE894",
    plans: [
      {
        title: "Activities",
        data: [
          "Choose activities according to your interests",
          "Explore Singapore's major attractions",
          "Take photographs and enjoy the experience",
        ],
      },
      {
        title: "Relax",
        data: [
          "Enjoy the evening at your own pace",
        ],
      },
    ],
  },

  {
    day: 4,
    title: "Sentosa Experience",
    image: singapore,
    color: "83D3FC",
    plans: [
      {
        title: "Visit",
        data: [
          "Explore Sentosa Island",
          "Enjoy the beach and island atmosphere",
        ],
      },
      {
        title: "Activities",
        data: [
          "Choose activities based on your preferences",
          "Spend the day at your own pace",
        ],
      },
    ],
  },

  {
    day: 5,
    title: "Shopping and Relaxation",
    image: singapore,
    color: "C7E788",
    plans: [
      {
        title: "Shopping",
        data: [
          "Explore popular shopping areas",
          "Shop for souvenirs and gifts",
        ],
      },
      {
        title: "Relax",
        data: [
          "Enjoy a relaxed day",
          "Explore Singapore at your own pace",
        ],
      },
    ],
  },

  {
    day: 6,
    title: "Departure Day",
    image: singapore,
    color: "F2A7B3",
    plans: [
      {
        title: "Departure",
        data: [
          "Pack your belongings",
          "Check out from the hotel",
          "Prepare for your flight home",
        ],
      },
    ],
  },
];